...
(async()=>{
 ...
 game.epoch=2;game.wake={key:'sheriff',index:1,mine:true,deadline:60000};signals.observe(game,31000);await new Promise(r=>setTimeout(r,0));assert.ok(plays.at(-1).url.endsWith('02-wake-up.mp3'));const wakeSound=audios.filter(a=>a.url.endsWith('02-wake-up.mp3')).at(-1),beforeTurn=plays.length;assert.equal(typeof wakeSound.onended,'function');wakeSound.onended();await new Promise(r=>setTimeout(r,0));assert.equal(plays.length,beforeTurn+1,'role wake plays once, followed by your-turn');assert.ok(plays.at(-1).url.endsWith('01-your-turn.mp3'),'active night role hears your-turn after the wake call');signals.observe(game,50001);assert.ok(plays.at(-1).url.endsWith('03-ten-seconds.mp3'));count=plays.length;signals.observe(game,55500);signals.observe(game,60000);assert.equal(plays.length,count,'warning fires once, never at expired deadline');
 ...
 count=plays.length;game.epoch++;game.wake={key:'pause',index:2,mine:false,deadline:65000};signals.observe(game,61000);assert.equal(plays.length,count+1,'end of personal wake asks player to sleep before the silent pause');assert.ok(plays.at(-1).url.endsWith('04-sleep.mp3'));
 game.epoch++;game.wake={key:'mafia',index:2,mine:false,deadline:90000};signals.observe(game,66000);assert.equal(plays.length,count+1,'sleep cue is not duplicated immediately after the quiet pause');
 ...
 console.log('Signals passed: supplied MP3 voices and byte-identical embedded delivery, single role wake plus your-turn cue, sleep-before-pause, ten-second warning, preview, saved volume, deduplication and mute.');
