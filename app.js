...
<style>
:root{--void:#15170f;--void-2:#1d2016;--paper:#e9e3d0;--ink:#211d13;--hazard:#d3a026;--danger:#a63d2f;--moss:#5b6b48;--ok:#5b8a4a;}
*{box-sizing:border-box; margin:0; padding:0;}
body{background:radial-gradient(ellipse at top,#1f2317 0%,var(--void) 60%);color:var(--paper);font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;min-height:100vh;padding:12px 8px 30px;}
...
.panel{background:var(--void-2);border:1px solid #33361f;border-radius:6px;padding:14px;margin-bottom:16px;}
.btn{display:inline-block;font-family:'Arial Black',sans-serif;text-transform:uppercase;letter-spacing:0.05em;font-size:14px;font-weight:600;padding:12px 20px;border-radius:4px;border:none;cursor:pointer; transition: 0.2s; touch-action:manipulation;}
...
input, select { width:100%;padding:12px;margin:8px 0; background: var(--void); border: 1px solid #4a4e28; color: var(--paper); border-radius: 4px; font-size: 16px; font-family:inherit;}
...
.player-list li { padding: 12px; background: var(--void); margin: 6px 0; border-radius: 4px; display: flex; justify-content: space-between; align-items: center; flex-wrap:wrap; gap:8px;}
...
.badge-ready{background:var(--ok);color:white;}
.badge-notready{background:#555;color:#ccc;}
.badge-muted{background:var(--danger);color:white;}
.badge-timeout{background:#7a5c1e;color:white;}
.player-name{font-weight:600;}
.player-actions{display:flex;gap:6px;flex-wrap:wrap;}
.settings-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;}
.settings-field label{display:block;font-size:12px;color:#b7b190;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.03em;}
.settings-field.wide{grid-column:1/-1;}
.settings-field.wide label{display:inline-block;text-transform:none;font-size:14px;color:var(--paper);}
.round-type-row{display:flex;align-items:center;gap:10px;margin-bottom:6px;}
.round-type-row span{width:80px;flex-shrink:0;font-size: 13px;}
.readonly-settings{font-size:14px;line-height:1.8;}
.scenario-card{background:var(--void);border:1px solid #4a4e28;border-radius:6px;padding:16px;margin-bottom:10px;cursor:pointer;transition:0.2s;}
.scenario-card:hover{border-color:var(--hazard);}
.prop-list{list-style:none;margin:10px 0;}
.prop-list li{padding:10px;background:var(--void);margin:6px 0;border-radius:4px;border-left:3px solid var(--moss);}
.prop-list li.bonus{border-left-color:var(--hazard);}
.prop-tag{font-size:10px;text-transform:uppercase;color:var(--hazard);display:block;margin-bottom:3px;}
.prop-list li.special-unused{border-left-color:#777; filter:grayscale(0.7); opacity:0.9;}
.prop-list li.special-used{border-left-color:#e0c04a; background:#2a2410;}
.events-feed{max-height:220px; overflow-y:auto;}
.seat-grid{display:flex; flex-wrap:wrap; gap:8px;}
.seat-grid .btn{min-width:80px;}
.chat-box{display:flex;flex-direction:column;height:280px;}
.chat-messages{flex:1;overflow-y:auto;background:var(--void);border-radius:4px;padding:10px;margin-bottom:10px;}
.chat-message{margin-bottom:6px;font-size:14px;}
.chat-message .sender{color:var(--hazard);font-weight:bold;}
.chat-input-row{display:flex;gap:8px;}
.chat-input-row input{margin:0;flex:1;}
.countdown-banner{background:var(--hazard);color:var(--ink);text-align:center;padding:16px;border-radius:6px;margin-bottom:16px;font-family:'Arial Black',sans-serif;font-size:20px;text-transform:uppercase;}
...
.host-strip{background:var(--void); border:2px solid #9b7fd4; border-radius:6px; padding:10px 14px; margin-bottom:14px; display:flex; align-items:center; gap:10px;}
.ptable-grid{display:grid; grid-template-columns:repeat(auto-fill, minmax(160px,1fr)); gap:10px;}
.ptable-card{background:var(--void); border:1px solid #4a4e28; border-radius:10px; padding:10px; display:flex; flex-direction:column; gap:8px; min-height:120px;}
.ptable-card.speaking{border-color:var(--hazard); box-shadow:0 0 0 2px var(--hazard) inset;}
.ptable-card.nominated{border-color:var(--danger);}
.ptable-card.eliminated{opacity:0.45; filter:grayscale(1);}
.ptable-card-head{display:flex; justify-content:space-between; align-items:center; gap:8px;}
.ptable-avatar{width:32px; height:32px; border-radius:50%; background:#2a2d1c; border:1px solid #4a4e28; flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:17px; line-height:1;}
.ptable-name{font-weight:600; font-size:13px; text-align:right; overflow-wrap:anywhere;}
.ptable-card-body{flex:1; display:flex; flex-direction:column; gap:6px; align-items:flex-start;}
#myCardPanel{min-height:220px;}
.catastrophe-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:1000;display:flex;align-items:center;justify-content:center;padding:20px;}
.catastrophe-box{background:var(--void-2);border:3px solid var(--danger);border-radius:12px;padding:30px;max-width:700px;max-height:80vh;overflow-y:auto;box-shadow:0 0 60px rgba(166,61,47,0.5);}
.catastrophe-box h2{color:var(--danger);text-align:center;margin-bottom:16px;}
.catastrophe-box p{font-size:16px;line-height:1.6;margin-bottom:20px;}
.moderation-log{max-height:300px;overflow-y:auto;font-size:13px;}
.moderation-log-item{padding:8px;background:var(--void);margin:4px 0;border-radius:4px;border-left:3px solid var(--danger);}
.ready-indicator{display:inline-block;padding:4px 10px;border-radius:12px;font-size:12px;font-weight:bold;background:var(--ok);color:white;margin-left:8px;}
.ready-indicator.partial{background:#7a5c1e;}
.complaint-btn{background:transparent;border:1px solid #666;color:#aaa;font-size:10px;padding:2px 6px;border-radius:3px;cursor:pointer;}
.complaint-btn:hover{border-color:var(--danger);color:var(--danger);}
</style>
</head>
<body>
...
function renderCustomizationPicker() {
  ...
  return `<h3 style="margin-top:4px;">Аватар</h3> <div style="display:flex; flex-wrap:wrap; gap:6px;">${avatarsHtml}</div> <h3 style="margin-top:12px;">Цвет ника</h3> <div style="display:flex; flex-wrap:wrap;">${swatches(me.color, 'actionSetColor')}</div> <h3 style="margin-top:12px;">Цвет обводки карточки</h3> <div style="display:flex; flex-wrap:wrap;">${swatches(me.outline_color, 'actionSetOutlineColor')}</div>`;
}
...
// ========== ЖАЛОБЫ ==========
async function dbFileComplaint(roomCode, targetId, reason) {
  const { error } = await supabaseClient.from('moderation_log').insert({
    room_code: roomCode, actor_id: state.playerId, target_id: targetId,
    action: 'complaint', reason: reason
  });
  if (error) console.error('Ошибка подачи жалобы:', error);
}

async function dbFetchModerationLog(roomCode) {
  const { data, error } = await supabaseClient.from('moderation_log')
    .select('*').eq('room_code', roomCode).order('created_at', { ascending: false }).limit(50);
  if (error) { console.error(error); return []; }
  return data || [];
}
...
function renderLobby() {
  ...
  document.getElementById('app').innerHTML = `
    ...
    ${isHost ? `<div class="panel"><h2>Журнал модерации</h2><div id="moderationLogPanel" class="moderation-log"><p class="muted-note">Загрузка...</p></div></div>` : ''}
    <div class="panel">
      <h2>Чат</h2>
      <div class="chat-box">
  ...
  if (isHost) { regenerateRoundBlocks(); loadModerationLog(); }
...
async function loadModerationLog() {
  const el = document.getElementById('moderationLogPanel');
  if (!el) return;
  const logs = await dbFetchModerationLog(state.currentRoomCode);
  if (logs.length === 0) { el.innerHTML = '<p class="muted-note">Пока нет записей.</p>'; return; }
  const nameOf = id => (state.players.find(p => p.id === id) || {}).name || 'Бывший игрок';
  el.innerHTML = logs.map(l => {
    const actor = nameOf(l.actor_id);
    const target = nameOf(l.target_id);
    const actionLabel = l.action === 'kick' ? '🚫 Кик' : l.action === 'mute' ? '🔇 Мут' : l.action === 'timeout' ? '⏱️ Таймаут' : l.action === 'complaint' ? '⚠️ Жалоба' : l.action;
    return `<div class="moderation-log-item"><b>${escapeHtml(actor)}</b> → <b>${escapeHtml(target)}</b>: ${actionLabel}${l.reason ? ' (' + escapeHtml(l.reason) + ')' : ''}<br><span class="muted-note">${new Date(l.created_at).toLocaleString()}</span></div>`;
  }).join('');
}
...
function renderPlayerRow(p, room, isHost) {
  ...
  return `<li>
    ...
    ` : (!isHost && !isHostRow ? `
      <span class="player-actions">
        <button class="complaint-btn" onclick="actionFileComplaint('${p.id}', '${escapeHtml(p.name)}')">⚠️ Жалоба</button>
      </span>
    ` : '')}
  </li>`;
}

async function actionFileComplaint(targetId, targetName) {
  const reason = prompt(`Жалоба на ${targetName}. Опишите причину:`);
  if (!reason) return;
  await dbFileComplaint(state.currentRoomCode, targetId, reason);
  alert('Жалоба отправлена ведущему.');
}
...
function renderGameTable() {
  ...
  // Индикатор готовности к переходу
  const alivePlayers = state.players.filter(p => p.id !== room.host_id && p.is_alive !== false);
  const readyIndicator = `<span class="ready-indicator partial" id="phaseReadyIndicator">0 / ${alivePlayers.length}</span>`;
  ...
  document.getElementById('app').innerHTML = `
    ...
    <div class="panel" style="border-left:6px solid ${meta.color}; text-align:center;">
      <div style="font-size:14px; letter-spacing:0.08em; text-transform:uppercase; color:${meta.color};">${meta.icon} ${escapeHtml(meta.label)} · Раунд ${room.current_round || 1} ${readyIndicator}</div>
...
function renderFinalPhaseTable() {
  ...
  if (isHost && room.current_phase === 'awaiting_verdict') {
    hostControls = `
      ...
      ${!room.catastrophe_read ? `<button class="btn btn-danger" style="margin-top:10px;" onclick="actionReadCatastrophe()">📜 Зачитать описание катастрофы</button>` : '<p class="muted-note" style="margin-top:8px;">✓ Описание катастрофы уже было зачитано.</p>'}
...
async function updateGameDynamic() {
  ...
  updatePhaseReadyIndicator();
}

function updatePhaseReadyIndicator() {
  const el = document.getElementById('phaseReadyIndicator');
  if (!el) return;
  const room = state.room;
  const alivePlayers = state.players.filter(p => p.id !== room.host_id && p.is_alive !== false);
  let readyCount = 0;
  if (room.current_phase === 'nomination') {
    const nominations = room.nominations || {};
    readyCount = alivePlayers.filter(p => nominations[p.id]).length;
  } else if (room.current_phase === 'voting') {
    // Подсчитаем через загрузку количества голосов (уже есть в loadVoteProgress)
    return;
  } else {
    readyCount = alivePlayers.length; // Для других фаз считаем всех готовыми
  }
  el.textContent = `${readyCount} / ${alivePlayers.length}`;
  el.className = 'ready-indicator' + (readyCount >= alivePlayers.length ? '' : ' partial');
}
...
// ========== ЗАЧИТАТЬ КАТАСТРОФУ ==========
async function actionReadCatastrophe() {
  const room = state.room;
  if (!room.scenario_id) return alert('Сценарий не выбран.');
  const { scenario } = await dbFetchScenarioDetail(room.scenario_id);
  if (!scenario) return alert('Не удалось загрузить сценарий.');

  // Показываем описание катастрофы всем игрокам через оверлей
  showCatastropheOverlay(scenario);

  // Помечаем, что катастрофа уже была зачитана
  await dbUpdateRoom(state.currentRoomCode, { catastrophe_read: true });
  await dbInsertEvent(state.currentRoomCode, room.current_round, 'neutral',
    'Ведущий зачитал описание катастрофы.', null, false);
  refreshEventsFeed();
}

function showCatastropheOverlay(scenario) {
  const overlay = document.createElement('div');
  overlay.className = 'catastrophe-overlay';
  overlay.innerHTML = `
    <div class="catastrophe-box">
      <h2>⚠️ ${escapeHtml(scenario.title)}</h2>
      <p>${escapeHtml(scenario.catastrophe_description)}</p>
      <button class="btn btn-danger" onclick="this.closest('.catastrophe-overlay').remove()" style="width:100%;">Понятно</button>
    </div>
  `;
...
async function actionResetToLobby() {
  ...
  await dbUpdateRoom(state.currentRoomCode, {
    ...
    final_reveal_unlocked: false, verdict: null, verdict_percent: null, catastrophe_read: false
  });
  state.lastRenderedView = null;
