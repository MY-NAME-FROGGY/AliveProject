...
nomination:    { label: 'Выставление',          icon: '👉', color: '#d3a026', durationKey: 'nomination' }, // ИСПРАВЛЕНО: был discussion
...
if (!key) return 0;
const seconds = state.room?.settings?.phase_seconds?.[key];
return seconds ? parseInt(seconds) : 60;
}
const state = {
...
phase_seconds: { reveal: 60, discussion: 180, nomination: 60, defense: 30, voting: 60 }, // ДОБАВЛЕНО nomination
...
if (isHost && room.phase === 'starting') {
     ...
     if (notReady) {
         ...
         await dbUpdateRoom(state.currentRoomCode, {
             phase: 'game', current_round: 1, current_phase: 'reveal', reveal_turn_index: 0,
 ...
 if (state.view === 'lobby') {
     ...
     const key = room.current_phase + '|' + room.current_round + '|' + (room.defense_index || 0) + '|' + (room.reveal_turn_index || 0);
     ...
     // Обновление игрового чата
     state.chat = await dbFetchGameChat(state.currentRoomCode);
...
document.getElementById('app').innerHTML = `<h1>ОСТАТЬСЯ <span>В ЖИВЫХ</span></h1> <div class="hazard-strip"></div> <div id="db-warning" class="warning"></div> <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px;"> <div class="panel"> <h2>Создать комнату</h2> <input id="hostName" placeholder="Ваше имя (Ведущий)" maxlength="20"> <button class="btn btn-primary" style="width:100%; margin-top:10px;" onclick="actionCreateRoom()">Создать</button> </div> <div class="panel"> <h2>Присоединиться</h2> <input id="joinCode" placeholder="Код комнаты (4 буквы/цифры)" maxlength="4" style="text-transform:uppercase;"> <input id="joinName" placeholder="Ваше имя" maxlength="20"> <button class="btn btn-primary" style="width:100%; margin-top:10px;" onclick="actionJoinRoom()">Войти</button> </div> </div>
`;
...
document.getElementById('app').innerHTML =  `<h1>ОСТАТЬСЯ <span>В ЖИВЫХ</span></h1> <div class="hazard-strip"></div> <div id="phaseBanner"></div> <div class="panel" style="text-align:center;"> <h3>Код комнаты для друзей:</h3> <div class="code-box">${room.code}</div> </div> <div class="panel"> <div class="section-title"><h2>Сценарий</h2> ${isHost ?
` <button class= "btn btn-ghost btn-sm " onclick= "openCatalog() " >${room.scenario_id ? 'Сменить' : 'Выбрать'} </button > `: ''} </div> <p id="scenarioSummary">${room.scenario_id ? 'Загрузка...' : 'Сценарий ещё не выбран.'}</p> <div id="scenarioDetailBtn">${isHost && room.scenario_id ?
` <button class= "btn btn-ghost btn-sm " onclick= "openScenarioDetail('${room.scenario_id}') " >Подробнее </button > `: ''}</div> </div> 
<div class="panel">
    <h2>Выбор места</h2>
    <p class="muted-note">Выберите свободное место за столом. Номер места будет отображаться рядом с вашим именем.</p>
    <div id="seatPicker" style="display:flex; gap:10px; flex-wrap:wrap; margin-top:10px;"></div>
</div>
<div class="panel" id="settingsPanel"> <h2>Настройки игры</h2> <div id="settingsContent">${isHost ? renderSettingsEditable(settings) : renderSettingsReadonly(settings)}</div> </div> <div class="panel"> <div class="section-title"><h2>Участники</h2><span id="playerCount" class="muted-note"></span></div> <ul class="player-list" id="playersList"></ul> </div> <div class="panel"> <h2>Чат</h2> <div class="chat-box"> <div class="chat-messages" id="chatMessages"></div> <div class="chat-input-row"> <input id="chatInput" placeholder="Сообщение..." onkeydown="handleChatKey(event)"> <button class="btn btn-primary btn-sm" onclick="actionSendChat()">➤</button> </div> </div> </div> <div style="display:flex; gap:10px; margin-top:20px; flex-wrap:wrap;"> <button class="btn btn-primary" id="readyToggleBtn" onclick="actionToggleReady()"></button> ${isHost ?
` <button class= "btn btn-danger " onclick= "actionStartGame() " >Начать игру </button > `: ''} <button class="btn btn-ghost" onclick="actionLeaveRoom()">← Выйти в меню</button> </div>
` ;
...
return `<div class="settings-grid"> <div class="settings-field"><label>Мин. игроков</label><input type="number" min="1" id="setMin" value="${s.min_players ?? 4}"></div> <div class="settings-field"><label>Макс. игроков</label><input type="number" min="1" id="setMax" value="${s.max_players ?? 12}"></div> <div class="settings-field"><label>Нужно выживших</label><input type="number" min="1" id="setSurvivors" value="${s.target_survivors ?? 3}"></div> <div class="settings-field"><label>Кол-во раундов</label><input type="number" min="1" id="setRounds" value="${s.rounds ?? 6}" onchange="regenerateRoundBlocks()"></div> <div class="settings-field"><label>Открытие, сек</label><input type="number" min="1" id="setReveal" value="${s.phase_seconds?.reveal ?? 60}"></div> <div class="settings-field"><label>Обсуждение, сек</label><input type="number" min="1" id="setDiscussion" value="${s.phase_seconds?.discussion ?? 180}"></div> <div class="settings-field"><label>Выставление, сек</label><input type="number" min="1" id="setNomination" value="${s.phase_seconds?.nomination ?? 60}"></div> <div class="settings-field"><label>Оправдание, сек</label><input type="number" min="1" id="setDefense" value="${s.phase_seconds?.defense ?? 30}"></div> <div class="settings-field"><label>Голосование, сек</label><input type="number" min="1" id="setVoting" value="${s.phase_seconds?.voting ?? 60}"></div> <div class="settings-field wide"> <label><input type="checkbox" id="setPrivateChat" style="width:auto;display:inline-block;margin-right:6px;vertical-align:middle;" ${s.private_chat_enabled ? 'checked' : ''}>Разрешить личные чаты между игроками</label> </div> </div> <h3 style="margin-top:14px;">Характеристики по раундам</h3> <p class="muted-note">На каждый раунд — сколько характеристик можно открыть и какого типа каждая (любая или конкретная категория).</p> <div id="roundBlocksContainer"></div> <button class="btn btn-primary btn-sm" style="margin-top:10px;" onclick="actionSaveSettings()">Сохранить настройки</button>
`;
}
...
return `<div class="readonly-settings"> Игроков: ${s.min_players ?? '?'}–${s.max_players ?? '?'} · Нужно выживших: ${s.target_survivors ?? '?'}<br> Раундов: ${s.rounds ?? '?'}<br> ${roundSummary ? '<div class="muted-note">' + roundSummary + '</div>' : ''} Фазы: открытие ${s.phase_seconds?.reveal ?? '?'}с · обсуждение ${s.phase_seconds?.discussion ?? '?'}с · выставление ${s.phase_seconds?.nomination ?? '?'}с · оправдание ${s.phase_seconds?.defense ?? '?'}с · голосование ${s.phase_seconds?.voting ?? '?'}с<br> Личные чаты: ${s.private_chat_enabled ? 'включены' : 'выключены'} </div>
`;
}
...
html += `<div style="background:var(--void); border-radius:4px; padding:8px 10px; margin-bottom:8px;"> <div style="display:flex; align-items:center; gap:10px;"> <strong style="width:80px; flex-shrink:0;">Раунд ${i + 1}</strong> <label class="muted-note">характеристик: <input type="number" min="1" max="10" id="roundCount_${i}" value="${count}" onchange="regenerateRoundSlots(${i})" style="width:60px; display:inline-block; margin:0 0 0 4px;"></label> </div> <div id="roundSlots_${i}" style="display:flex; gap:6px; flex-wrap:wrap; margin-top:6px;"></div> </div>
`;
}
...
` +
CATEGORY_LIST.map(c => `<option value="${c}" ${val === c ? 'selected' : ''}>${CATEGORY_LABELS[c]}</option>
`).join('') +
`</select>
`;
}
...
const countEl = document.getElementById('playerCount');
 ...
 renderChatMessages();
 renderPhaseBanner();
 renderSeatPicker(); // Рендер выбора места
}
...
return  `<li> <span> <span class="player-name">${p.seat_number ? `<span style="color:var(--hazard);">[${p.seat_number}]</span> ` : ''}${escapeHtml(p.name)}${isMe ? ' (Вы)' : ''}</span> ${isHostRow ? '<span class="host-badge">Ведущий</span>' : ''} <span class="badge ${p.is_ready ? 'badge-ready' : 'badge-notready'}">${p.is_ready ? 'Готов' : 'Не готов'}</span> ${p.is_muted ? '<span class="badge badge-muted">Мут</span>' : ''} ${timedOut ? '<span class="badge badge-timeout">Таймаут</span>' : ''} </span> ${isHost && !isMe ?
...
el.innerHTML = state.chat.map(m => `<div class="chat-message"><span class="sender">${escapeHtml(nameOf(m.sender_id))}:</span> ${escapeHtml(m.text)}</div>
`).join('');
...
document.getElementById('app').innerHTML =  `<h1>ОСТАТЬСЯ <span>В ЖИВЫХ</span></h1> <div class="hazard-strip"></div> <button class="btn btn-ghost" onclick="backToLobby()">← Назад в лобби</button> <h2 style="margin-top:16px;">Каталог сценариев</h2> ${state.catalog.length === 0 ? '<p class="muted-note">Сценариев пока нет в базе.</p>' : ''} ${state.catalog.map(s =>
` <div class= "scenario-card " onclick= "openScenarioDetail('${s.id}') " > <h3 style= "margin:0; " >${escapeHtml(s.title)} </h3 > </div > `).join('')}
` ;
}
...
document.getElementById('app').innerHTML =  `<h1>ОСТАТЬСЯ <span>В ЖИВЫХ</span></h1> <div class="hazard-strip"></div> <button class="btn btn-ghost" onclick="backToLobby()">← Назад в лобби</button> <div class="panel" style="margin-top:16px;"> <h2>${escapeHtml(scenario.title)}</h2> <p>${escapeHtml(scenario.catastrophe_description)}</p> </div> <div class="panel"> <h3>Стартовые свойства бункера</h3> <ul class="prop-list">${base.map(p =>
` <li > <span class= "prop-tag " >База </span >${escapeHtml(p.text)} </li > `).join('')}</ul> <h3>Дополнительные свойства (в игре выпадет 2–4 из ${bonus.length})</h3> <ul class="prop-list">${bonus.map(p =>
` <li class= "bonus " > <span class= "prop-tag " >Бонус </span >${escapeHtml(p.text)} </li > `).join('')}</ul> </div> ${isHost ?
` <button class= "btn btn-primary " onclick= "confirmScenario('${scenario.id}') " >Выбрать этот сценарий </button > `: ''}
` ;
}
...
let phaseBody = '';
 if (room.current_phase === 'reveal') {
     const players = state.players.filter(p => p.id !== room.host_id);
     const activePlayer = players[room.reveal_turn_index || 0];
     if (activePlayer) {
         phaseBody = `<p>Сейчас ход игрока: <strong>${escapeHtml(activePlayer.name)}</strong></p>`;
         if (activePlayer.id !== state.playerId) {
             phaseBody += `<p class="muted-note">Ожидание, пока игрок откроет характеристики и завершит ход.</p>`;
         } else {
             phaseBody += `<p class="muted-note">Ваш ход! Откройте характеристики или завершите ход.</p>`;
         }
     } else {
         phaseBody = `<p>Все игроки сделали свой ход. Ведущий может перейти к следующей фазе.</p>`;
     }
 ...
 document.getElementById('app').innerHTML = `
     ...
     <div class="game-layout">
         ...
         <div class="game-main">
             ...
             <div class="panel" id="gameChatPanel">
                 <h2>Чат</h2>
                 <div class="chat-box">
                     <div style="margin-bottom:8px; display:flex; align-items:center; gap:10px;">
                         <label style="font-size:12px; color:#b7b190; margin:0;">Кому:</label>
                         <select id="gameChatTarget" style="margin:0; width:auto; display:inline-block; padding:6px;">
                             <option value="">Всем</option>
                             ${state.players.filter(p => p.id !== state.playerId).map(p => `<option value="${p.id}">${escapeHtml(p.name)}</option>`).join('')}
                         </select>
                     </div>
                     <div class="chat-messages" id="gameChatMessages"></div>
                     <div class="chat-input-row">
                         <input id="gameChatInput" placeholder="Сообщение..." onkeydown="if(event.key==='Enter') actionSendGameChat()">
...
const myNomination = nominations[state.playerId];
 ...
 if (hostEl) {
     hostEl.innerHTML = hostP ? `<div class="host-strip">
         ...
         <div style="flex:1;"><span class="player-name">${hostP.seat_number ? `<span style="color:var(--hazard);">[${hostP.seat_number}]</span> ` : ''}${escapeHtml(hostP.name)}${hostP.id === state.playerId ? ' (Вы)' : ''}</span></div>
     ...
     listEl.innerHTML = state.players.filter(p => p.id !== room.host_id).map(p => {
         ...
         return `<div class="ptable-card${isSpeaking ? ' speaking' : ''}${isNominated ? ' nominated' : ''}">
             <div class="ptable-card-head">
                 <div class="ptable-avatar"></div>
                 <div class="ptable-name">${p.seat_number ? `<span style="color:var(--hazard);">[${p.seat_number}]</span> ` : ''}${escapeHtml(p.name)}${isMe ? ' (Вы)' : ''}</div>
             </div>
...
const room = state.room;
// Фильтрация личных событий: видят только адресат и ведущий
const visibleEvents = events.filter(e => {
    if (!e.target_id) return true;
    return e.target_id === state.playerId || state.playerId === room.host_id;
});
if (visibleEvents.length === 0) { el.innerHTML = ' <li class="muted-note">Пока ничего не произошло.</li>'; return; }
el.innerHTML = visibleEvents.map(e => {
const targetName = e.target_id ? (state.players.find(p => p.id === e.target_id) || {}).name : null;
return  `<li>${eventIcon(e.type)} ${escapeHtml(e.text)}${targetName ?
` <span class="muted-note">(${escapeHtml(targetName)})</span> `: ''}</li>
` ;
}).join('');
}
...
if (card.length === 0) {
 ...
 const players = state.players.filter(p => p.id !== room.host_id);
 const activePlayer = players[room.reveal_turn_index || 0];
 const isMyTurn = room.current_phase === 'reveal' && activePlayer && activePlayer.id === state.playerId;
 ...
 const itemsHtml = card.map(c => {
     ...
     if (!c.revealed) {
         if (isMyTurn) {
             ...
             body = `<div style="font-weight:bold;">${escapeHtml(c.text)}</div>
                     <div class="muted-note" style="font-size:11px;">Цену характеристики вы узнаете только после открытия.</div>`;
             if (check.ok) {
                 body += `<button class="btn btn-sm btn-primary" style="margin-top:5px;" onclick="actionRevealTrait('${c.id}')">Открыть</button>`;
             } else {
                 body += `<div class="muted-note" style="font-size:11px; margin-top:3px;">${escapeHtml(check.reason)}</div>`;
             }
         } else {
             body = `<i class="muted-note">Скрыто</i>`;
         }
 ...
 const hasUnrevealed = card.some(c => !c.revealed);
 const sumHtml = hasUnrevealed ? '' : `<p class="muted-note">Сумма баланса карточки: ${sum > 0 ? '+' : ''}${sum} (ориентир — ближе к 0, не строго)</p>`;
 ...
 el.innerHTML = `
     ...
     ${isMyTurn ? `<button class="btn btn-danger" style="margin-top:14px; width:100%;" onclick="actionEndRevealTurn()">Завершить ход →</button>` : ''}
 `;
}
...
el.innerHTML =  `<h3>${escapeHtml(scenario.title)}</h3> <p>${escapeHtml(scenario.catastrophe_description)}</p> <h4 style="margin-top:10px;">Стартовые свойства</h4> <ul class="prop-list">${base.map(p =>
` <li > <span class= "prop-tag " >База </span >${escapeHtml(p.text)} </li > `).join('')}</ul> ${revealedBonus.length ?
` <h4 style= "margin-top:10px; " >Открытые доп. свойства </h4 >
 <ul class= "prop-list " >${revealedBonus.map(p =>  `<li class="bonus"><span class="prop-tag">Бонус</span>${escapeHtml(p.text)}</li>
` ).join('')} </ul >
 `: ''}
` ;
}
...
? revealedItems.map(b => `<li class="bonus"><span class="prop-tag">Бонус</span>${escapeHtml(b.text)}</li>
`).join('')
...
if (phase === 'defense' && defenseIdx < nominees.length - 1) {
 ...
 await dbUpdateRoom(state.currentRoomCode, {
     ...
     defense_index: 0, reveal_turn_index: nextPhase === 'reveal' ? 0 : room.reveal_turn_index,
...
await dbUpdateRoom(state.currentRoomCode, {
phase: 'lobby', countdown_ends_at: null, current_round: 1, current_phase: 'reveal', reveal_turn_index: 0,
phase_ends_at: null, phase_running: false, phase_paused_remaining: null,
...
nomination: parseInt(document.getElementById('setNomination').value) || 60,
...
// ---------- Игровой чат ----------
async function dbFetchGameChat(code) {
    const { data, error } = await supabaseClient.from('chat_messages')
        .select('*').eq('room_code', code)
        .order('created_at', { ascending: true }).limit(100);
    if (error) return [];
    return data.filter(m => {
        if (!m.recipient_id) return true; // Общие видят все
        if (state.playerId === state.room.host_id) return true; // Ведущий видит все
        return m.sender_id === state.playerId || m.recipient_id === state.playerId; // ЛС видят только отправитель и получатель
    });
}
...
async function actionSendGameChat() {
    ...
    const targetEl = document.getElementById('gameChatTarget');
    const recipientId = targetEl ? targetEl.value || null : null;
    
    if (recipientId && !state.room.settings?.private_chat_enabled && state.playerId !== state.room.host_id) {
        return alert('Личные чаты отключены ведущим.');
    }
...
function renderGameChatMessages() {
    ...
    el.innerHTML = state.chat.map(m => {
        const recipientName = m.recipient_id ? nameOf(m.recipient_id) : null;
        const isPrivate = !!m.recipient_id;
        const prefix = isPrivate ? `<span style="color:#a63d2f; font-size:11px;">[ЛС для ${escapeHtml(recipientName)}]</span> ` : '';
        return `<div class="chat-message">${prefix}<span class="sender">${escapeHtml(nameOf(m.sender_id))}:</span> ${escapeHtml(m.text)}</div>`;
    }).join('');
    el.scrollTop = el.scrollHeight;
}
...
function renderSeatPicker() {
    ...
    if (!el) return;
    const max = state.room.settings?.max_players || 12;
    const taken = {};
    state.players.forEach(p => { if (p.seat_number) taken[p.seat_number] = p.id; });
    
    let html = '';
    for (let i = 1; i <= max; i++) {
        const isTaken = taken[i] && taken[i] !== state.playerId;
        const isMine = taken[i] === state.playerId;
        const cls = isMine ? 'btn-primary' : 'btn-ghost';
        const disabled = isTaken && !isMine ? 'disabled style="opacity:0.4; cursor:not-allowed;"' : '';
        html += `<button class="btn ${cls} btn-sm" style="width:40px; height:40px; font-size:16px;" ${disabled} onclick="actionSelectSeat(${i})">${i}</button>`;
    }
    el.innerHTML = html;
}

async function actionSelectSeat(seat) {
    await supabaseClient.from('players').update({ seat_number: seat }).eq('id', state.playerId).eq('room_code', state.currentRoomCode);
}

// ---------- Завершение хода в фазе открытия ----------
async function actionEndRevealTurn() {
    const room = state.room;
    if (room.current_phase !== 'reveal') return;
    const nextIndex = (room.reveal_turn_index || 0) + 1;
    await dbUpdateRoom(state.currentRoomCode, { reveal_turn_index: nextIndex });
}
