...
export const DEFAULTS={daySeconds:180,nominationSeconds:30,voteSeconds:45,nightSeconds:30,bonusSeconds:20,resultSeconds:10,soloLastAlive:true,cameraMode:'hidden',strictCameraCheck:true,gameMode:'auto',counts:{mafia:2,sheriff:1,doctor:1},maxPlayers:24};
...
export function validateSettings(settings,players=null){
 ...
 if(!['hidden','open'].includes(out.cameraMode))fail('Неверный режим камер');if(typeof out.strictCameraCheck!=='boolean')fail('Неверная настройка строгой проверки камер');out.soloLastAlive=out.soloLastAlive!==false;return out;
}
...
function phase(s,next,now){s.phase=next;s.epoch++;s.phaseStarted=now;s.cameraGraceUntil=now+45000;s.deadline=next==='finished'||next==='lobby'?null:now+(next==='night_pause'?5000:s.settings[{night:'nightSeconds',night_bonus:'bonusSeconds',day:'daySeconds',nomination:'nominationSeconds',voting:'voteSeconds',result:'resultSeconds'}[next]]*1000);}
...
export function changeLobby(original,id,command,data={},camera=false){const s=clone(original),p=member(s,id);if(s.phase!=='lobby')fail('Настройки состава доступны в лобби');
 if(command==='ready'){if(isModerator(s,p))fail('Ведущий не занимает место игрока');if(data.ready&&s.settings.strictCameraCheck!==false&&!camera)fail('Для готовности подключите камеру');p.ready=!!data.ready;}
